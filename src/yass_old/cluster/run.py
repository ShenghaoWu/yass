...
def run(scores, spike_index):
    ...
    path_to_triage_score = path.join(CONFIG.data.root_folder, 'tmp/triage_score.npy')
    path_to_triage_spike_index = path.join(CONFIG.data.root_folder, 'tmp/triage_spike_index.npy')
    if os.path.exists(path_to_spike_train_clear):
        score = np.load(path_to_triage_score)
        spike_index = np.load(path_to_triage_spike_index)
    else:
        score_triage, spike_index_triage = triage(scores, spike_index,
                                    CONFIG.cluster.triage.nearest_neighbors,
                                    CONFIG.cluster.triage.percent)
        np.save(path_to_triage_score,score)
        np.save(path_to_triage_spike_index,spike_index)
    ...
    if CONFIG.cluster.method == 'location':
        ...
        spike_train = run_cluster_location(scores_subsampled, spike_index_subsampled, CONFIG)
        Time['s'] += (datetime.datetime.now()-_b).total_seconds()


	spike_train = recover_clear_spikes(score_triage, spike_index_triage)
