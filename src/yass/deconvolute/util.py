...
def calculate_temp_temp(temporal_features, spatial_features):
    ...
    temp_temp = np.zeros((n_templates, n_templates, n_shifts, n_shifts, 2*waveform_size-1), 'float32')
...
def calculate_temp_temp_parallel(templates_selected, temporal_features, spatial_features):
    ...
    temp_temp = np.zeros((n_templates, len(templates_selected), n_shifts, n_shifts, 2*waveform_size-1), 'float32')
    n_pad = waveform_size - 1
    
    temporal_conv = np.zeros((2*waveform_size-1, n_templates, len(templates_selected)),dtype='float32')
    for j1 in range(n_features):
        for j2 in range(n_features):
            for s1 in range(n_shifts):
                for s2 in range(n_shifts):
		    print (' temp_temp loop indexes: ',j1, '/', n_features, '  ', j2, '/', n_features, '   ', s1,'/', n_shifts,'   ', s2, '/', n_shifts, '   ',datetime.datetime.now().strftime('%H:%M:%S'))
                    temp1 = temporal_features[:, s1, :, j1]
